...
int ler_registro(FILE *binario, reg_dados *registro){
    // como 'registro' foi recebido como ponteiro (*), usamos a seta '->' para acessar seus campos 
    ...
    if(fread(&registro->status_removido, sizeof(char), 1, binario) == 0){
        return 0; // significa que o arquivo acabou e retorna 0
    }
    ...
    if(registro->status_removido == '1'){ // significa que o registro foi apagado depois de ler o 1º byte
        fseek(binario, 79, SEEK_CUR);     // como já leu o 1º pula o restante (79 bytes a partir da posição atual) indo para a próxima linha
                                          // significa que o arquivo foi apagado e a linha será pulada
    }
    ...
    // ------- 👀
    fread(&registro->tamNomeEstacao, sizeof(int), 1, binario);
        if (registro->tamNomeEstacao > 0) { 
            registro->nomeEstacao = malloc(registro->tamNomeEstacao + 1); 
            fread(registro->nomeEstacao, sizeof(char), registro->tamNomeEstacao, binario);    
